    ...
    for num in range(1, 50):
        ...
        for i, row in def_elements.iterrows():
            ...
            for genus in genuses:
                relations.append({ "em1Text": definiendum, "em2Text": genus, "label": 'hyponym/hypernym' })
            ...
            for j, row2 in rel_rel_frame.iterrows():
                ...
                if rel_tok.replace(" ", "") in sentence.replace(" ", ""):
                    rel_frame = row2['REL_FRAME']
                    if not pd.isna(rel_frame):
                        rel_frames = rel_frame.split('|')
                        
                        for frame in rel_frames:
                            rel_tok = rel_tok.replace(frame, '')
                            relations.append({ "em1Text": frame, "em2Text": rel_tok, "label": row2['REL_LABEL'] })
            
            item = { "sentText": sentence, "relationMentions": relations }
...
annotated_definitions_path = "../../data/EN/"
output_file = "../../data/EN/termframe2spn4re.json"
