...
from utils import read_predictions, AverageMeter, formulate_gold, num_metric, overlap_metric, metric
...
class Trainer(nn.Module):
    ...
    def predict(self, eval_loader, tokens, read_instance):
        self.model.eval()
        # print(self.model.decoder.query_embed.weight)
        prediction, gold = {}, {}
        read = []
        with torch.no_grad():
            ...
            for batch_id in range(total_batch):
                ...
                gen_triples = self.model.gen_triples(input_ids, attention_mask, info)
                prediction.update(gen_triples)
                read.append(read_predictions(tokens[start:end], gen_triples, read_instance))
